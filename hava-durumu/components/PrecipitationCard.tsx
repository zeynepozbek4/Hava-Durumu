...
import { PrecIcon } from "../icons/PrecIcon";

interface PrecCardProps {
  prec: number;
  rain: number;
  snow: number;
  color: string;
...
const PrecCard: React.FC<PrecCardProps> = ({ prec, rain, snow, color }) => {
  return (
    <Card
      sx={{
        width: 160,
    ...
    >
      <CardContent sx={{ position: "relative", paddingBottom: "3rem" }}>
        ...
        >
          ...
          >
            <PrecIcon pop={prec} rain={rain} snow={snow} />
          </Box>
        </Box>
      </CardContent>
    </Card>
